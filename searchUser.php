...
<?php
	...
	from user where role_level = 3 order by reg_time desc limit $doffset,$dpagesize";
//		echo $exec;
	...
	//计算司机总数
	$dcount_result = mysql_query("SELECT count(*) as count FROM user where role_level=3");
	...
	if ($dpagenum > 1) {
		for($i=1;$i<=$dpagenum;$i++) {
			if($i==$dp) {
				...
				echo ' <a href="searchUser.php?dp=',$i,'">',$i,'</a>';
			}
		}
	}
